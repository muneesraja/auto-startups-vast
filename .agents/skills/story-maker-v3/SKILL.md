...
Independent Replicate character, location, and object generations run with bounded
concurrency. Set `REPLICATE_IMAGE_CONCURRENCY` (default `2`) to control in-flight
jobs and `REPLICATE_MIN_INTERVAL_SEC` (default `1`) to pace request starts. For
low-credit Replicate accounts, set both values conservatively to stay within the
account's rate limits; retryable throttling and read-timeout failures are retried.
...
## Direct Ref2VA production (no storyboard sheets)

Use this path when the user explicitly asks for direct H3 Ref2VA renders and does
not want storyboard sheets. Do not quietly fall back to a storyboard-derived
reference image. Every generation remains 5–15 seconds, but its references are
semantic production assets rather than a panel grid.

### Required asset bible

Before writing or rendering a direct Ref2VA generation, lock each referenced
entity into exactly one asset category:

- **Style** — the canonical animation look: eye scale, facial proportions,
  materials, lighting, palette, and explicit anti-realism rules.
- **Characters** — one turnaround/pose reference per named character or
  recurring group; speaking or close-up characters need their own reference.
- **Animals** — the lead animal, supporting pack, friendly animal, and crowd
  species are separate assets whenever the story differentiates their roles.
- **Vehicles** — an exterior vehicle asset is never a substitute for its
  interior location.
- **Locations** — separate empty 4K 360-degree equirectangular plates for
  each reusable geography, such as a cockpit, cargo bay, river glade, or
  battle clearing.
- **Props** — product, cargo, food, tool, and stunt props. A user-supplied
  branded product is attached directly and its condition locks are repeated in
  every prompt; never regenerate it as a generic substitute.

Do not combine unrelated characters, vehicles, interiors, and props into one
reference board merely to work around a reference-image limit. If a generation
needs style + subject + location/product, use a workflow with three genuinely
wired image inputs.

### Direct Ref2VA gates

1. **Workflow gate.** Validate the selected ComfyUI workflow before any paid
   render: `MiniMaxH3ReferenceToVideo` must expose the exact number of linked
   `ref_images.ref_image_N` inputs required by the reference plan. A visible
   but unlinked UI socket does not count.
2. **Asset gate.** Generate and visually approve the style, character, animal,
   vehicle, location, and prop assets. Check 4K location resolution, 360-degree
   geography, identity separation, and product fidelity.
3. **Proof gate.** Render only generation 1 using the direct Ref2VA prompt and
   its exact physical input order. Confirm style adherence, subject visibility,
   audio, and product condition before any batch render.
4. **Batch gate.** Recheck each generation's prompt labels against the physical
   image order, then render the remaining clips and concatenate only approved
   outputs.

### Direct Ref2VA prompt contract

Use `minimax-h3-prompter`'s six-section Ref2VA format. The prompt's
`<Picture N>` labels must match the uploaded input order exactly. Put the global
style reference first. When an asset is derived from that style but the original
style image is not physically attached, state that limitation and do not claim
it is a direct reference. Include the explicit no-realism lock in every prompt
when the approved style is cartoon 3D.
