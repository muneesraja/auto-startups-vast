...
_last_call = 0.0
_MIN_INTERVAL = float(os.getenv("REPLICATE_MIN_INTERVAL_SEC", "2"))
_MAX_RETRIES = max(5, int(os.getenv("REPLICATE_MAX_RETRIES", "6")))


def _throttle() -> None:
    """Pace Replicate request starts while allowing bounded in-flight concurrency."""
    global _last_call
    with _rate_lock:
        now = time.monotonic()
...
def _is_retryable_error(exc: Exception) -> bool:
    text = str(exc).lower()
    return any(token in text for token in ("429", "throttl", "timed out", "timeout", "temporarily unavailable"))


def _run_prediction(client, model: str, input_data: dict):
    for attempt in range(_MAX_RETRIES + 1):
        try:
            ...
            if attempt >= _MAX_RETRIES or not _is_retryable_error(exc):
                raise
            delay = max(5.0, 3.0 * (2 ** attempt))
            print(f"⏳ [replicate] throttled/retryable error ({exc}); waiting {delay:.1f}s (attempt {attempt+1}/{_MAX_RETRIES})...")
            time.sleep(delay)
    raise RuntimeError("Replicate prediction retry loop exhausted")
    ...
    *,
    ...
    try:
        output = _run_prediction(
            client,
            _model_id(),
            _build_input(final_prompt, resolution, size=size, quality=quality),
        )
