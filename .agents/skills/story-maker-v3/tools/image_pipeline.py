...
def run_image_jobs(jobs: list[Callable[[], Any]], max_workers: int | None = None) -> list[Any]:
    """Run independent image jobs with bounded concurrency and stable result order."""
    workers = max_workers or config.REPLICATE_IMAGE_CONCURRENCY
...
class AssetRegistry:
    ...
    def inspiration_asset(self, iid: str) -> dict:
        return self.data.setdefault("inspirations", {}).setdefault(iid, {"output_path": "", "fal_image_url": ""})

    def object_asset(self, oid: str) -> dict:
        return self.data.setdefault("objects", {}).setdefault(oid, {"output_path": "", "fal_image_url": ""})
...
def build_sheet_ref_urls(
    ...
    *,
    ...
    """Ordered refs for a storyboard sheet edit: location -> previous sheet -> characters -> objects -> inspirations."""
    ...
    for oid in sorted(registry.data.get("objects", {})):
        url = ensure_asset_url(registry.object_asset(oid), provider=resolved)
    ...
    for iid in sorted(registry.data.get("inspirations", {})):
        url = ensure_asset_url(registry.inspiration_asset(iid), provider=resolved)
        if url and url not in urls:
            urls.append(url)
        if len(urls) >= limit:
            break
...
def generate_character_sheet(
    ...
    *,
    ...
    """Generate one character sheet (T2I or edit with refs). Returns the registry entry."""
    ...
    if ref_image_urls:
        result = _check(
            generate_grok_edit(
                prompt_text, ref_image_urls, out_path,
...
def generate_location_lock(
    ...
    *,
    ...
    """Generate one location lock plate (T2I empty stage or edit with refs). Returns registry entry."""
...
def generate_object_sheet(
    ...
    *,
    ...
    """Generate one objects/props asset sheet (T2I empty stage / 4K or edit with refs). Returns registry entry."""
    backend = provider or config.get_image_provider()
    if not prompt_text:
    ...
    if ref_image_urls:
        result = _check(
            generate_grok_edit(
                prompt_text, ref_image_urls, out_path,
                size=config.BACKGROUND_IMAGE_SIZE,  # 3840x2160 (4K landscape)
                quality=config.REPLICATE_SHEET_QUALITY,
                provider=backend,
            ),
            f"object sheet {oid}",
    ...
    entry = registry.object_asset(oid)
    _registry_entry_from_result(entry, result)
    registry.save()
    return entry
