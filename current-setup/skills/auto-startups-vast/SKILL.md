...
|---|---|
| `~/.hermes/skills/creative/story-to-video` | `~/repos/auto-startups-vast/current-setup/skills/story-to-video` |
| `~/.hermes/skills/runpod-ai` | `~/repos/auto-startups-vast/current-setup/skills/runpod-ai` |
| `~/.hermes/skills/vast-ai` | `~/repos/auto-startups-vast/current-setup/skills/vast-ai` |
| `~/.hermes/skills/workflow-researcher` | `~/repos/auto-startups-vast/current-setup/skills/workflow-researcher` |
...
2. Move it to the repo:
   ```bash
   mv ~/.hermes/skills/<category>/<skill-name> ~/repos/auto-startups-vast/current-setup/skills/<skill-name>
   ```
3. Replace the original with a symlink:
   ```bash
   ln -s ~/repos/auto-startups-vast/current-setup/skills/<skill-name> ~/.hermes/skills/<category>/<skill-name>
   ```
4. Commit and push:
   ```bash
   cd ~/repos/auto-startups-vast
   git add current-setup/skills/<skill-name>/
...
2. Remove from repo and commit:
   ```bash
   cd ~/repos/auto-startups-vast
   git rm -r current-setup/skills/<skill-name>/
...
2. **`git add` on a directory symlink** stages the symlink itself, not the files inside. If you see `new file: current-setup/skills/<name>` (without trailing `/`), it's a symlink — remove it and use real files instead.
...
# Should output: /root/repos/auto-startups-vast/current-setup/skills/story-to-video
```