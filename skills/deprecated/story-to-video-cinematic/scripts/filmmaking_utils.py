...
def find_latest_file(directory, prefix, extension):
    """
    Finds the file in the given directory that starts with the given prefix,
    ends with the given extension, and has the highest numerical counter.
    Returns (absolute_path, basename).
    If no matching file is found, returns (None, None).
    """
    import re
    ...
    # Matches prefix_00004_.png or prefix_00004.png
    pattern = re.compile(rf"^{re.escape(prefix)}_(\d+)_?\.(?i:{re.escape(extension)})$")
    ...
    for filename in os.listdir(directory):
        ...
        if match:
            ...
            if num > best_num:
                best_num = num
                best_file = filename
        elif filename.startswith(prefix) and filename.endswith(f".{extension}"):
            # Fallback for non-counter filenames (e.g. copied/predefined files)
            if best_num == -1:
                best_file = filename

    if best_file:
        return os.path.join(directory, best_file), best_file
    return None, None
