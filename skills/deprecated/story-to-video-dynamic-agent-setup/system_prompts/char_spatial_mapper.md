...
1. The **FF prompter** uses it to write Flux Klein prompts that explicitly anchor each character sheet reference to a screen region — preventing identity swaps on multi-character shots.
2. The **LF prompter** uses it the same way for the LF (last-frame) image, anchoring char sheets + the FF image reference to specific on-screen figures.
...
- `reference_index`: 1-based integer indicating which positional char-sheet slot this character occupies in `reference_images` (1 = first ref in the list, 2 = second, ...). Downstream FF and LF prompters will emit refs in the exact order you specify here.
