# Story Maker V3 — project context

Turns a high-level story plus a target duration into `final_film.mp4`. **Claude Code is the brain** (authors every markdown/text artifact, runs validators, reads storyboard sheets for the vision step). **Python is the hands** (image gen, Minimax H3 render, concat). Python makes **zero LLM calls**. There is no ADK and no LiteLLM. Video backend is **Minimax H3 R2V via ComfyUI**: each generation is at most **15 seconds** and renders from one storyboard sheet plus a timeline prompt, with native stereo audio.

Operational runbook (commands, validators, gates): [`SKILL.md`](SKILL.md).
...
- Episode run: `outputs/story-maker-v3/<story>/epi-N/`
- Shared assets (never wiped): `outputs/story-maker-v3/<story>/assets/` (`characters/{cid}.png`, `locations/{lid}.png`)
...
- **A1–A4 (planning, no image spend):** write → validate → fix. Agent 1 develops the story and locks `char_NN` / location ids. Agent 2 breaks into scenes (targets sum within 15% of `TARGET`). Agent 3 storyboards generations with `panel_grid` and shots. Agent 4 authors image prompt files (cast-lock: only scene cast ids).
...
- **C1 (vision):** Read the **drawn** sheet; write Minimax timeline prompts. Describe characters by appearance, never `char_NN`. Shot times are generation-local (start at 0.0). Direct audio inline.
- **GATE 2:** stop for video-prompt review before paid GPU hours. Runbook rule, not code-enforced.
...
| Minimax prompt bible | [`assets/minimax-h3-prompt-bible.md`](assets/minimax-h3-prompt-bible.md) |
