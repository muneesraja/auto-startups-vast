...
def render_scene(
    ...
    if res.get("status") != "success":
        _ntfy(f"[story-maker-v4] scene {scene_id} concat failed: {res.get('message')}")
        raise RuntimeError(f"scene concat failed for {scene_id}: {res.get('message')}")
    _ntfy(f"[story-maker-v4] scene {scene_id} complete -> {scene_mp4}")
...
def main() -> int:
    ...
    if len(scene_mp4s) == 1:
        ...
        _ntfy(f"[story-maker-v4] final film complete -> {final}")
    else:
        res = concat_videos(scene_mp4s, final)
        if res.get("status") != "success":
            _ntfy(f"[story-maker-v4] final concat failed: {res.get('message')}")
