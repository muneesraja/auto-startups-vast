...
def test_h3_reference_budget_is_enforced_before_upload():
    from tools.minimax_workflow import validate_reference_budget

    assert validate_reference_budget([], [], []) is None
    assert validate_reference_budget([f"identity-{i}.webp" for i in range(8)], [], []) is None
    assert "at most 9 image" in validate_reference_budget([f"identity-{i}.webp" for i in range(9)], [], [])
    assert "at most 3 video" in validate_reference_budget([], ["a.mp4"] * 4, [])
    assert "at most 3 audio" in validate_reference_budget([], [], ["a.wav"] * 4)
