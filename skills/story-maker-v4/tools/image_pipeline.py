...
def ensure_asset_url(entry: dict, *, provider: str | None = None) -> str | None:
    ...
    # A persisted remote asset needs no provider credentials unless it must be
    # re-hosted; this also keeps planning and validation offline-safe.
    if url and not (local_path and os.path.isfile(local_path)):
        return url
...
def build_sheet_ref_urls(
    ...
    *,
    ...
    if provider:
        ...
        try:
            resolved = config.get_storyboard_image_provider()
        except ValueError:
            # Reference ordering can be inspected without credentials; an
            # upload still validates the selected provider when one is needed.
            resolved = config.DEFAULT_IMAGE_PROVIDER
