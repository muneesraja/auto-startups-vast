...
with a storyboard sheet as the primary reference image plus a timeline prompt.
...
custom nodes are required:
  ...
  - optional identity images, tail videos, and audio references are wired into
    their dynamic reference slots in deterministic attachment order;
...
# MiniMax H3 reference-generation limits. The storyboard counts as one image.
...
def validate_reference_budget(
    ...
    """Return a user-facing error when H3's reference-input limits are exceeded."""
    image_count = 1 + len(extra_images or [])  # storyboard sheet is required
    ...
    if image_count > MAX_REFERENCE_IMAGES:
        return f"H3 accepts at most {MAX_REFERENCE_IMAGES} image references including the storyboard sheet (got {image_count})"
    if video_count > MAX_REFERENCE_VIDEOS:
        return f"H3 accepts at most {MAX_REFERENCE_VIDEOS} video references (got {video_count})"
    if audio_count > MAX_REFERENCE_AUDIOS:
        return f"H3 accepts at most {MAX_REFERENCE_AUDIOS} audio references (got {audio_count})"
    if total > MAX_REFERENCE_FILES:
        return f"H3 accepts at most {MAX_REFERENCE_FILES} mixed reference files (got {total})"
    return None
...
def render_generation(
    *,
    ...
    reference_error = validate_reference_budget(
        extra_reference_paths, extra_reference_video_paths, extra_reference_audio_paths,
    )
    if reference_error:
