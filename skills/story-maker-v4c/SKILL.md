---
name: story-maker-v4c
version: 3.0.0-c
description: "Story-to-video skill (grounded + commercial). Claude Code is the brain (authors markdown/text, validators, vision) and Python is the hands (image gen, Minimax H3 render, concat). No ADK, no LiteLLM. Video backend: Minimax H3 R2V via ComfyUI — each <=15s generation renders from one storyboard sheet + a Hailuo 3.0 timeline prompt, native stereo audio. Use when the user asks for story-maker-v4c, grounded creature coverage, or commercial H3 ads."
triggers:
  - story-maker-v4c
  - story-maker v4c
---

# Story Maker V4c — grounded / commercial, MiniMax H3

Architecture overview: [`PROJECT_CONTEXT.md`](PROJECT_CONTEXT.md). Shared
authoring specs: [`prompts/creature_behavior.md`](prompts/creature_behavior.md),
[`prompts/coverage.md`](prompts/coverage.md),
[`prompts/commercial_ad.md`](prompts/commercial_ad.md) (ads only),
...
**v4c is MiniMax H3 / Hailuo 3.0 only** (same ComfyUI R2V hands as v3). It has
...
- `ffmpeg` for concat.
- Python deps: `pip install -r skills/story-maker-v4c/requirements.txt`
...
Output layout: `outputs/story-maker-v4c/<story>/epi-N/`; per-story shared assets at
...
All commands run from `skills/story-maker-v4c/`. Let `RUN=outputs/story-maker-v4c/<name>`
...
[`prompts/story_developer.md`](prompts/story_developer.md): `## Tone` (grounded
default), visible emotion, `creature_role:` on every non-human, expand/shrink to
target, `## Characters` and `## Locations`. Ads: one purpose per scene. No
...
[`prompts/coverage.md`](prompts/coverage.md): the scene split into
`## Generation gK — a-b s` blocks (each 5-15s, contiguous, summing to the
scene's `target_seconds`), each with `panel_grid` and `### Shot` blocks
(contiguous, panels in reading order, **one** H3 camera sentence, audio +
dialogue). **The 15s rule is load-bearing: a shot that does not fit in the
current generation moves whole to the next one.**

**5–8 micro-shots per 15s generation** only for coverage situation
`play_comedy`. Otherwise follow situation + subject (establish, discover,
danger, protect, emotion, scale). Then:
...
and [`assets/minimax-h3-prompt-bible.md`](assets/minimax-h3-prompt-bible.md):
I2V lock (`@image1` / storyboard at 0.00s), takeaway, appearance + creature
locks (never `char_NN`), `Timeline` with `integrated_multimodal_description`
and one `SHOT n — a–b s (Continuous Shot)` per storyboard shot in
**generation-local seconds**, one camera sentence per shot, `(S1)` +
`<d>[English]`, `Hard cinematic cut.` between shots (omit on a true one-take),
`Final frame:`, `overall_soundscape`, `non_diegetic_music`, `Negative Prompt`.
Do not default to 1.5–3.0s viral cuts. If GATE 2 needs a rewrite, change one
failure category. Then:
...
10. **Emotion, creature role/state, situation/subject camera.** Grounded default.
    Missing takeaway, mixed sound layers, first-frame-only H3 prompts, or a
    stuffed campaign in one 15s gen will look dumb. Follow coverage.md;
    micro-shots only for `play_comedy`.
