...
numbers / labels on the page). Paint each panel's **subject and situation**
from the storyboard (human CU, animal through cover, product insert,
tree-as-scale) — do not invent a new camera for every cell. Fold in the
generation's shots panel-by-panel in row-major order — each panel naming its
`characters_present`, framing, and beat — emphasizing visible progressive
motion between adjacent panels of a shot and the continuation from the previous
generation's last panel. Readable faces; species-true bodies. Style follows
Tone, not always Pixar.
