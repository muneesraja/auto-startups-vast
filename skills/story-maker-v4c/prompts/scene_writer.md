...
- **Beat line.** One concise sentence (still a single `beat:` value — no new
  keys). Name the central visible action, the **stakes**, the **emotional
  register**, and — when an animal drives the beat — that animal's
  **role+state** from [`creature_behavior.md`](creature_behavior.md). Ads: one
  job/purpose for this scene, not a whole campaign.
