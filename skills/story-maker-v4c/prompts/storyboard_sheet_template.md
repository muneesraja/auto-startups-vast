...
sheet + character sheets as edit references. Follow [`coverage.md`](coverage.md):
paint the storyboard's subject and situation, not a new camera in every cell.
...
1. **Grid + format line.**
   ...
   kind." Dense micro-shot grids (`3x3`, `2x4`, …) only for `play_comedy`.
2. **High-level scene synopsis + emotional arc.** Two or three sentences:
   place, characters, and what faces/bodies do in time order. Name each
   animal's role/state. Example: first a wide of the path (establish), then
   the leopard through cover (discover), then a medium two-shot of the mother
   as shield (protect).
3. **Subject / situation (not a new camera every panel).** Paint what the
   storyboard called for: human CU, animal through cover, object/product
   insert, tree-as-scale. Adjacent panels of one shot keep the same coverage
   and advance pose. Readable faces; species-true bodies (no cute mash).
4. **Visual style + continuity locks.** Match Tone (`grounded` cinematic 3D
   or `stylized`). Warm or cool lighting, character descriptions, sequence
...
5. **Negative/forbidden line.** no text, numbers, timecodes, labels, captions,
   ...
   frames, decorative layouts, overlapping panels, cells of different sizes,
   camera-smiling wildlife, zoo-portrait in a danger beat, dutch canopy.
