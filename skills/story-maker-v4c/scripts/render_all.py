...
def render_scene(run_dir: str, scene_id: str, *, seed: int,
    ...
    for gen in gens:
        ...
        if result.get("status") != "success":
            _ntfy(f"[story-maker-v4c] {scene_id}/{gid} render failed: {result.get('message', result)}")
        ...
        _ntfy(f"[story-maker-v4c] {scene_id}/{gid} render complete -> {out_path}")
        clip_paths.append(out_path)
    ...
    if res.get("status") != "success":
        _ntfy(f"[story-maker-v4c] scene {scene_id} concat failed: {res.get('message')}")
        raise RuntimeError(f"scene concat failed for {scene_id}: {res.get('message')}")
    _ntfy(f"[story-maker-v4c] scene {scene_id} complete -> {scene_mp4}")
    return scene_mp4
...
def main() -> int:
    ...
    if len(scene_mp4s) == 1:
        ...
        _ntfy(f"[story-maker-v4c] final film complete -> {final}")
    else:
        res = concat_videos(scene_mp4s, final)
        if res.get("status") != "success":
            _ntfy(f"[story-maker-v4c] final concat failed: {res.get('message')}")
