# Story Maker V4p — Deep Architecture
...
│   │   └── char_02.webp
│   ├── locations/
│   │   ├── loc_jungle_stream.webp   (4K wide-angle establishing plate, generated once)
│   │   ├── loc_jungle_stream_360.mp4 (H3 360° environment video sweep)
│   │   ├── loc_jungle_stream_walls/ (Extracted contradiction-free wall reference plates)
│   │   │   ├── wall_000deg.webp     (Front wall plate)
│   │   │   ├── wall_090deg.webp     (Right wall plate)
│   │   │   ├── wall_180deg.webp     (Reverse-angle wall plate)
│   │   │   └── wall_270deg.webp     (Left wall plate)
...
### Background Coherence Subsystem (360° Source Video & Wall Extraction)

```
   ┌────────────────────────────────────────────────────────────┐
   │ 1. Establishing Plate (4K)                                 │
   │    assets/locations/<lid>.webp                             │
   └─────────────────────────────┬──────────────────────────────┘
                                 │
                                 ▼
   ┌────────────────────────────────────────────────────────────┐
   │ 2. MiniMax H3 360° Environment Video                       │
   │    Fixed central pivot, smooth 360° pan, held lighting     │
   │    assets/locations/<lid>_360.mp4                          │
   │    (Sole definitive spatial reference for the space)       │
   └─────────────────────────────┬──────────────────────────────┘
                                 │
                                 ▼
   ┌────────────────────────────────────────────────────────────┐
   │ 3. Deterministic Frame Extraction                          │
   │    tools/background_extractor.py                           │
   │    assets/locations/<lid>_walls/wall_*.webp                │
   │    (Front, Right, Back/Reverse, Left, Landmark-aligned)    │
   └─────────────────────────────┬──────────────────────────────┘
                                 │
            ┌────────────────────┴────────────────────┐
            ▼                                         ▼
   ┌───────────────────────────┐             ┌───────────────────────────┐
   │ Storyboard Sheet Refs     │             │ Ref2VA Prompts            │
   │ Targeted wall plate       │             │ <Picture 2> wall plate    │
   │ attached per camera angle │             │ locks background on cuts  │
   └───────────────────────────┘             └───────────────────────────┘
```

**The Core Rule:** *It doesn't have to be an orbit video specifically; as long as you create a "source for backgrounds that can be extracted later without contradictions," that seems to do the trick.*

### Previz with Claude Code + Blender Clay Models
For complex camera cut sequences, Claude Code generates a headless Blender Python script creating a simple gray clay model (boxes + cylinders for walls, props, characters) with keyframed camera cuts. This allows locking the cut timestamps and camera facings to the exact matching frames of the 360° video.
...
class AssetRegistry:
    ...
    #   locations:  {loc_01:  {output_path, fal_image_url, walls: {...}, source_video: "..."}}
    ...
    def object(oid) -> dict
    def sheet(sid) -> dict
    def location_walls_dir(lid) -> str
    def save() / load()
```
...
2. Resolves names via the registry (`resolve_ref_names()` — objects → locations/walls → characters → sheets)
...
│   ├── minimax-h3-prompt-bible.md    ← Distilled Ref2VA + base_en contract
│   └── official/                     ← Vendored MiniMax H3 prompt guides
