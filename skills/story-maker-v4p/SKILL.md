...
# Story Maker V4p — Claude-as-brain, Python-as-hands (Minimax H3)
...
- Python deps: `pip install -r skills/story-maker-v4p/requirements.txt`
  ...
  **no** litellm).
- H3 video prompts follow [`assets/minimax-h3-prompt-bible.md`](assets/minimax-h3-prompt-bible.md);
  official MiniMax guides are vendored in [`assets/official/`](assets/official/).
...
Output layout: `outputs/story-maker-v4p/<story>/epi-N/`; per-story shared assets at
...
5. assets/characters/*.png         (Python T2I, once, 4K) — build_images.py --assets-only
   ...
   assets/locations/*_360.mp4      (Python/H3 360° environment video sweep)
   assets/locations/*_walls/*.webp (Extracted contradiction-free wall reference plates)
...
All commands run from `skills/story-maker-v4p/`. Let `RUN=outputs/story-maker-v4p/<name>`
...
and the official six-section Ref2VA contract in
[`assets/minimax-h3-prompt-bible.md`](assets/minimax-h3-prompt-bible.md)
(full text: [`assets/official/`](assets/official/)). Sections:
`subject_definitions` / `summary` / `retention_analysis` /
`detailed_description` / `overall_soundscape` / `non_diegetic_music`.
`[Shot N] At MM:SS.mmm` timestamps in **generation-local seconds**.
Storyboard sheet = `<Picture 1>` (shot-planning, not an I2VA first frame).
If this generation will receive a 3s tail (`g2+`, or `g1` of a later scene),
declare `<Video 1>` and use `[reference generation + video continuation]`.
Dialogue: `<d>[English] ...</d>` with stable speaker IDs. Identity/count
locks as inline prose. Then:
...
14. **Contradiction-free backgrounds across cuts.** Backgrounds change when
    cuts switch because AI redraws them from scratch. To maintain rock-solid
    background consistency, shoot a 360° pan around the room with MiniMax H3
    as the sole definitive reference for the space, then extract single frames
    of the walls the camera faces for each cut. (*Universal rule: create a
    source for backgrounds that can be extracted later without contradictions.*)
    For cut planning and camera blocking, author a simple gray clay model using
    boxes and cylinders in Blender via Claude Code.
