# Director's guide (story-maker-v4p)
...
### Background Coherence Across Cuts (The 360° Rule)
...
As a workaround, the method of **first shooting the desired background in a consistent form with H3, then extracting just the needed walls later** proves remarkably effective:
1. **Definitive 360° source:** Start with a slightly wide-angle still image as the opening frame, create a single video panning 360° around the room from a fixed point with MiniMax H3, and use that as the sole definitive reference for the room.
2. **Wall extraction per cut:** For each cut, extract a single frame of the wall the camera is facing and use it as a reference image (`<Picture 2>`).
3. **The universal rule:** *It doesn't have to be an orbit video specifically; as long as you create a "source for backgrounds that can be extracted later without contradictions," that seems to do the trick.*

### Previz with Claude Code + Blender Clay Models
For camera positions and cut points, author a simple gray clay model using just boxes and cylinders in Blender and pass it as a layout previz video (the clay model script can be generated directly with Claude Code). For each cut point in the clay model, extract the matching wall angle from the 360° video to feed the renderer.
