...
Follow [`character_sheet_template.md`](character_sheet_template.md):
one 16:9 **documentation** identity sheet (not a redesign, not an expression
grid). Pull appearance/species/wardrobe from `developed_story.md`'s
`## Characters` entry for that `cid`.

Lock identity with a small named view pack, not "use the same character":

- Top row: four full-body views at the same scale and light — front, 3/4,
  **strict 90° profile** (must not look like a second 3/4), back.
- Bottom row: four **named distinct** close-ups (head-and-shoulders, costume /
  torso, signature accessory, one extra identity crop). Each area once; no
  repeated crops.
- The person or creature is the subject (no mannequin). Stable wardrobe unless
  the story changes it. Neutral studio backdrop. Frozen still. No typography.

Name a few silhouette-defining features (face, hair, palette, one accessory).
Do not use brand names (`Pixar`, `Disney`). End with the no-text clause.
Keep 16:9 (the backend uses `CHARACTER_SHEET_SIZE`).
...
Follow [`object_sheet_template.md`](object_sheet_template.md): a 4K
**documentation** prop sheet — same object, named panels, no repeats. Top row:
front, 3/4, strict profile, back. Bottom row: four distinct identity crops
(body, working part, grip/join, material). No A-pose. Pull `description` +
`appearance` from `developed_story.md`'s `## Objects` entry. End with the
no-text clause.
