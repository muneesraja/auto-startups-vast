...
- `camera_facing`: `toward_<landmark>`, `away_from_<landmark>`, or
  `along_<axis>`.
  - **Wall Reference Mapping:** When a 360° room video is available, `camera_facing`
    directly targets the extracted wall plate (e.g., `wall_toward_<landmark>.webp` or
    `wall_090deg.webp`). This guarantees background consistency when cutting across
    different walls.
...
## Previz Tip — Blender Gray Clay Model

For complex interiors or dynamic camera cuts:
- Ask Claude Code to generate a simple Python script for headless Blender (`blender -b -P layout.py`) creating a gray clay model using primitive boxes and cylinders (representing the room, furniture, and character blocking) with keyframed camera cuts.
- Exporting camera cut points and facings from this clay model allows you to match camera cuts directly to the extracted single frames of the 360° MiniMax room video.
- *Universal Principle:* It doesn't have to be an orbit video specifically; as long as you create a "source for backgrounds that can be extracted later without contradictions," that seems to do the trick.
