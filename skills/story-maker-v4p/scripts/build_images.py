...
def build_assets(reg: ip.AssetRegistry, scenes: dict) -> None:
    ...
    # Check for 360° background source videos and extract wall plates if present
    for lid in lids:
        l_path = reg.location_path(lid)
        video_360_path = os.path.join(os.path.dirname(l_path), f"{lid}_360.mp4")
        if os.path.isfile(video_360_path):
            from tools.background_extractor import extract_cardinal_walls, register_location_walls
            from tools.video_frames import get_video_duration
            walls_dir = reg.location_walls_dir(lid)
            dur = get_video_duration(video_360_path)
            extracted = extract_cardinal_walls(video_360_path, dur, walls_dir)
            if extracted:
                register_location_walls(reg, lid, extracted, video_path=video_360_path)
                print(f"  location {lid}: extracted {len(extracted)} wall reference plates from 360 video")
...
def build_sheets(reg: ip.AssetRegistry, scenes: dict, scene_id: str) -> None:
    ...
    for gen in gens:
        ...
        # Check if a targeted wall reference plate exists for this generation's facing
        wall_ref_url = None
        if loc and spatial and gid in spatial_gens:
            loc_data = reg.data.get("locations", {}).get(loc, {})
            walls = loc_data.get("walls", {})
            if walls:
                from tools.background_extractor import resolve_facing_wall
                sg = spatial_gens[gid]
                target_facing = sg.get("wall_reference", "")
                if not target_facing:
                    shots = sg.get("shots", [])
                    if shots and isinstance(shots, list) and isinstance(shots[0], dict):
                        target_facing = shots[0].get("camera_facing", "")
                if target_facing:
                    wpath = resolve_facing_wall(target_facing, walls)
                    if wpath and os.path.isfile(wpath):
                        wentry = loc_data.setdefault("_wall_entries", {}).setdefault(target_facing, {"output_path": wpath})
                        wall_ref_url = ip.ensure_asset_url(wentry)
        ...
        print(f"  sheet {scene_id}_{gid}: generating "
              f"(refs: loc={'yes' if attach_location else 'no'} "
              f"wall={'yes' if wall_ref_url else 'no'} prev={prev_sheet_id} "
...
def main() -> int:
    p = argparse.ArgumentParser(description="Build story-maker-v4p image media")
    ...
    p.add_argument("--extract-walls", default=None, help="extract wall plates from a 360 video for location id")
    args = p.parse_args()
    ...
    if args.extract_walls:
        lid = args.extract_walls
        l_path = reg.location_path(lid)
        video_360_path = os.path.join(os.path.dirname(l_path), f"{lid}_360.mp4")
        if not os.path.isfile(video_360_path):
            raise SystemExit(f"360 video not found for {lid}: {video_360_path}")
        from tools.background_extractor import extract_cardinal_walls, register_location_walls
        from tools.video_frames import get_video_duration
        walls_dir = reg.location_walls_dir(lid)
        dur = get_video_duration(video_360_path)
        extracted = extract_cardinal_walls(video_360_path, dur, walls_dir)
        register_location_walls(reg, lid, extracted, video_path=video_360_path)
        print(f"Extracted {len(extracted)} wall plates for {lid} into {walls_dir}")
        return 0
    ...
    if args.assets_only:
        ...
        return 0
    if not args.scene:
        raise SystemExit("pass --assets-only, --extract-walls <lid>, or --scene <id>")
