...
def test_ref2va_g1_does_not_warn_for_missing_video_label():
    res = validators.validate_video_prompt(REF2VA_PROMPT, _sb(), "g1")
    assert res.ok, res.errors
    assert not any("Video 1" in w for w in res.warnings)


REF2VA_PROMPT_G2 = textwrap.dedent("""
    subject_definitions:
    <Subject 1> is the toddler in the white onesie in <Picture 1>, with chubby cheeks and big eyes.
    <Subject 2> is the tiny green dinosaur in <Picture 1>, with large yellow eyes and a playful expression.
    <Picture 1> is a storyboard reference for [Shot 1], defining viewpoint, subject placement, and shot order.
    <Video 1> is the continuation source: the last 3 seconds of the previous generation.

    summary:
    [reference generation + video continuation] The target video continues from <Video 1> and shows the baby petting the dinosaur.
    ...
    detailed_description:
    The target video uses cinematic 3D CG with warm dusty half-light.
    [Shot 1] The shot continues from <Video 1>. The baby pets the smiling dinosaur. The camera holds a static shot, then zooms in with small amplitude at slow speed. Never generate duplicate characters or extra babies.

    overall_soundscape:
    Calm room tone, soft fabric, and a quiet dusty ambience throughout.

    non_diegetic_music:
    Sparse piano at a slow tempo, gradually decreasing in volume.
""").strip()


def test_ref2va_g2_requires_video_label():
    res = validators.validate_video_prompt(REF2VA_PROMPT_G2, _sb(), "g2")
    assert res.ok, res.errors
    assert not any("Video 1" in w for w in res.warnings)


def test_ref2va_g2_warns_when_tail_video_omitted():
    bad = REF2VA_PROMPT_G2.replace(
        "<Video 1> is the continuation source: the last 3 seconds of the previous generation.\n",
        "",
    ).replace(
        ...
        "<Video 1> (opening continuation): partially_preserved - motion and ending state carry into [Shot 1]; not a frame pin.\n",
        "",
    ).replace("continues from <Video 1>. ", "")
    res = validators.validate_video_prompt(bad, _sb(), "g2")
    assert res.ok, res.errors
    assert any("Video 1" in w for w in res.warnings)
    assert any("video continuation" in w for w in res.warnings)
