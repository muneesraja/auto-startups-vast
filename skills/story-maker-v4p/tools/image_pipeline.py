"""Image media pipeline for story-maker-v4p (the "hands").
...
class AssetRegistry:
    ...
    def location_walls_dir(self, lid: str) -> str:
        return os.path.join(self.assets_dir, "locations", f"{lid}_walls")
    ...
    def resolve_ref_name(self, name: str) -> str | None:
        ...
        Resolution order: objects → locations (and walls) → characters → sheets.
        ...
        for section in ("objects", "locations", "characters", "sheets"):
            ...
            if entry and entry.get("output_path"):
                ...
                if url:
                    return url
        # Check location wall reference plates (locations.<lid>.walls.<name>)
        for lid, loc_data in self.data.get("locations", {}).items():
            walls = loc_data.get("walls", {})
            wall_path = walls.get(name)
            if wall_path and os.path.isfile(wall_path):
                wall_entry = loc_data.setdefault("_wall_entries", {}).setdefault(name, {"output_path": wall_path})
                url = ensure_asset_url(wall_entry)
                if url:
                    return url
        return None
...
def build_sheet_ref_urls(
    ...
    *,
    ...
    wall_ref_url: str | None = None,
) -> list[str]:
    """Ordered refs for a storyboard sheet edit.

    Reference ordering (identity-first):
      prev sheet -> conditional location / wall plate -> chars -> extras

    For g1 (no prev sheet): location / wall plate -> chars -> extras.
    ...
    # Targeted wall plate takes precedence or complements general location
    if wall_ref_url and wall_ref_url not in urls:
        urls.append(wall_ref_url)
