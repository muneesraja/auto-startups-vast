"""Deterministic validators for story-maker-v4p (Minimax H3 backend).
...
Schemas enforced:
  ...
  video_prompt  -> per-generation 6-section Ref2VA prompt; [Shot N] count and
                   timestamps match the storyboard; labels defined; no char_NN;
                   warns if a tail-conditioned gen omits <Video 1>.
"""
...
def _generation_has_tail_ref(
    ...
    *,
    ...
    """True when render_all.py will attach the previous generation's 3s tail.

    g2+ of this storyboard always has a tail. g1 of scene s2+ has a cross-scene
    tail when a prior scene storyboard exists (or scene_id is s2+).
    """
    gens = [g.get("gen_id") for g in sb.get("generations", []) if g.get("gen_id")]
    if gen_id in gens and gens.index(gen_id) > 0:
        return True
    m = re.match(r"^s(\d+)$", (scene_id or "").strip(), re.I)
    if not m:
        return False
    n = int(m.group(1))
    if n <= 1:
    ...
    if run_dir:
        for i in range(1, n):
            if os.path.isfile(os.path.join(run_dir, f"storyboard_s{i}.md")):
                return True
        return False
    return True
...
def validate_video_prompt(
    ...
    *,
    ...
    if _generation_has_tail_ref(sb, gen_id, run_dir=run_dir, scene_id=scene_id):
        if "Video 1" not in defined_labels:
            res.warn(
                "generation has a tail ref_video; declare <Video 1> in subject_definitions"
            )
        if "video continuation" not in sections["summary"].lower():
            res.warn(
                "generation has a tail ref_video; summary should include 'video continuation'"
            )
...
def validate(artifact_path: str, schema: str, *, target_seconds: int | None = None,
    ...
    if schema == "video_prompt":
        ...
        return validate_video_prompt(
            text, sb, gid, run_dir=run_dir, scene_id=scene_id
        )
