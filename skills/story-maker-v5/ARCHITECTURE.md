...
8. [Image Generation Subsystem](#8-image-generation-subsystem)
   - [8.1 Storyboard Grid Topology & Defaults](#81-storyboard-grid-topology--default-layouts)
9. [Video Render Subsystem](#9-video-render-subsystem)
   - [9.1 The 15-Second Generation Constraint & How Continuous Films Are Achieved](#91-the-15-second-generation-constraint--continuous-film-engineering)
...
15. [Canonical End-to-End Generation Example](#15-canonical-end-to-end-generation-example)
...
- **Cell Size:** Each cell is **1920×720 pixels** (8:3 panoramic aspect ratio).
...
├──────────────┼──────────────┤   ├──────────┼──────────┼──────────┤
│   Panel 2    │   Panel 5    │   │ Panel 2  │ Panel 5  │ Panel 8  │
├──────────────┼──────────────┤   ├──────────┼──────────┼──────────┤
│   Panel 3    │   Panel 6    │   │ Panel 3  │ Panel 6  │ Panel 9  │
└──────────────┴──────────────┘   └──────────┴──────────┴──────────┘
```
...
2. **Sequential Tail-Conditioning (`ref_videos`):**
   ...
   - Once `g1.mp4` completes, Python runs `ffmpeg -sseof -3.0` to extract the final 3.0 seconds into `clips/sN/g1_tail.mp4`.
   - `g2` is dispatched to Minimax H3 with `ref_videos: [clips/sN/g1_tail.mp4]`. Minimax's UNet uses the tail video's final frames, actor velocities, and lighting state as initial condition vectors.
   - This eliminates jump-cuts, maintains emotional and physical inertia across the 15-second seams, and preserves lighting continuity.
3. **Cross-Scene Handoff:**
   - The final 3.0s tail of Scene N's last generation is passed to `g1` of Scene N+1 when continuous physical action connects them.
4. **Pacing & Shot Rhythm:**
   - A 15-second generation typically contains **3 to 6 shots** (1.5s to 3.5s per shot).
   - Video prompts enforce exact timestamp boundaries, camera motions, and synchronized foley/dialogue cues for each cut.
5. **Lossless Assembly:**
   - Python executes `ffmpeg` stream concatenation (`video_concat.py`) to merge generation clips into `scene_sN.mp4`, and all scenes into `final_film.mp4` with zero quality degradation or A/V desync.
...
1. **Target Model Infrastructure Matrix:**
   - **Still Images (Characters, Locations, Objects, Storyboards):** Generated using **OpenAI GPT-Image-2** (`openai/gpt-image-2` via Replicate or fal) at 3840×2160 (4K UHD) WebP quality 90. (Alternative local ComfyUI workflow on Vast AI: Flux 2 / FLUX Schnell).
