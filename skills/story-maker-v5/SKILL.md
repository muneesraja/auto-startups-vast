...
name: story-maker-v5
version: 5.0.0
description: "Production-ready story-to-video skill combining v3's canonical Ref2VA token-binding continuity and v4's animation screenplay format. Features dynamic shot pacing (2 minimum to 8 maximum per 15s generation), strict 3x2 (min) to 3x3 (max) storyboard grids, and tail-video conditioning for seamless multi-minute films."
triggers:
  - story-maker-v5
  - story-maker v5
---
...
with strict `<Subject N>`, `<Picture 1>`, and `<Video 1>` token bindings, native stereo
audio, and dynamic shot pacing (2 to 8 shots per generation).
...
**2 minimum to 8 maximum per generation** (with super high detail mandated for 2-shot scenes).
...
rendered tail (3s) as a `ref_video`. No bridge generations are used.
