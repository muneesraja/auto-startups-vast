...
async def instruction_provider(context: ReadonlyContext) -> str:
    ...
    additional_instr = (
        ...
        f"Use the structural skeleton below:\n\n{{blueprint_structure_json}}\n\n"
        f"And extract the visual details from the Director Script below:\n\n{{director_script_content}}\n\n"
        f"Additionally, use the FFLF Visual Composition Plan below to enrich the camera framing, character entries/positions, and visual deltas:\n\n{{fflf_plan_content}}"
    )
    
    full_prompt = system_prompt + additional_instr
    return await instructions_utils.inject_session_state(full_prompt, context)

blueprint_visuals_agent = LlmAgent(
