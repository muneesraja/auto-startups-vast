...
import json
from scripts.blueprint_projections import project_for_spatial_mapper
...
async def instruction_provider(context: ReadonlyContext) -> str:
    ...
    if raw_bp:
        ...
        projected = project_for_spatial_mapper(bp_dict)
        print(f"📉 [char_spatial_mapper] Projected blueprint: {len(projected)} chars (from {len(raw_bp) if isinstance(raw_bp, str) else 0} chars)")
    else:
        projected = "{}"
