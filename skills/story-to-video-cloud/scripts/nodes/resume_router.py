...
_RESUME_STEPS = [
    ...
    ("fflf_plan", "fflf_plan_content", "fflf_plan.json", "json"),
...
async def resume_router(ctx: Context) -> None:
    ...
    for route_name, state_key, filename, parser in _RESUME_STEPS:
        ...
        if state_key and parser == "text":
            ...
            if only_scenes and isinstance(data, dict) and "scenes" in data:
                filtered_scenes = [s for s in data["scenes"] if s.get("scene_id") in only_scenes]
                data["scenes"] = filtered_scenes
                print(f"🎯 [resume_router] Filtered {filename} scenes to only_scenes={only_scenes}")
