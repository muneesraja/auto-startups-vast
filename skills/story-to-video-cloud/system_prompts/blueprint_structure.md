...
1. Every shot duration must be an integer between 6 and 12 (inclusive).
...
7. `use_ff_as_lf_reference` should be `true` only for shots with `no-change` or `expression-shift` delta types where a near-identical FF/LF is required. Default is `false`.
8. `generate_background` is optional at the scene level (default `false`). If `true`, a corresponding `background_prompt` must be provided describing the environment with no characters.
