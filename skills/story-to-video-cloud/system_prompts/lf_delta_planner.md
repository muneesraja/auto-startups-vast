...
2. **Respect duration** (this is critical for avoiding video model hallucination):
   - **6-7s shots**: favor `pose-change`, `expression-shift`, or `particle-motion`. Moderate-deltas.
   - **8-9s shots**: favor `pose-change`, `camera-move`, or `env-shift`. Standard-deltas.
   - **10-12s shots**: MUST use `pose-change`, `camera-move`, or `env-shift`. These require the LARGEST visual delta — the LF must depict a clearly different moment in time from the FF. Combine with particle/environment changes for added visual distance.
3. **Plan environmental micro-motions**: Always identify environmental elements in the scene (e.g., sea, trees, sky, sand) and specify how they should evolve or change (e.g., waves shifting patterns/advancing, wind blowing leaves, background crabs/birds moving) regardless of the main delta type, to ensure visual life in the background.
4. **Check the scene-level distribution** before finalizing each shot's type. Reorder or swap types to satisfy the variety constraints.
5. **Continuation shots** (`continuation_from_previous == true`) should favor `pose-change` or `env-shift` to mask the visual transition from the previous video.
6. Output a single JSON object mapping shot_id → `delta_type`. No markdown, no commentary.
