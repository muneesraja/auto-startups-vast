...
def build_dynamic_workflow(template, shot_data, global_cfg):
    ...
    if ref_slots is None and builder_type not in ["flux_t2i", "ltx_i2v", "ltx_director", "ltx_fflf_seed_hunter", "ltx_fflf_flf2v", "ideogram_t2i", "flux_klein_edit", "flux_klein_edit_dynamic"]:
    ...
    if builder_type in ["flux_t2i", "ltx_i2v", "ltx_director", "ltx_fflf_seed_hunter", "ltx_fflf_flf2v", "ideogram_t2i", "flux_klein_edit", "flux_klein_edit_dynamic"]:
    ...
    if builder_type == "flux_klein_edit":
        ...
        workflow_str = workflow_str.replace('__FPS__', str(fps))
    elif builder_type == "ltx_fflf_flf2v":
        first_frame = shot_data.get("first_frame_image", "")
        last_frame = shot_data.get("last_frame_image", "")
        duration_seconds = shot_data.get("segment_duration", global_cfg.get("segment_duration", 3))
        fps = shot_data.get("fps", global_cfg.get("fps", 24))
        width = global_cfg.get("width", 1280)
        height = global_cfg.get("height", 720)
        motion_prompt = shot_data["prompt"]
        ...
        if "2103" in workflow:
            workflow["2103"]["inputs"]["value"] = motion_prompt
        if "2078" in workflow:
            workflow["2078"]["inputs"]["value"] = int(duration_seconds)
        if "2076" in workflow:
            workflow["2076"]["inputs"]["value"] = float(fps)
        if "2080" in workflow:
            workflow["2080"]["inputs"]["value"] = int(width)
        if "2079" in workflow:
            workflow["2079"]["inputs"]["value"] = int(height)
