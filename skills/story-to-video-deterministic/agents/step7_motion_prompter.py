...
async def instruction_provider(context: ReadonlyContext) -> str:
    ...
    additional_instr = (
        ...
        "    \"lf_image\": \"{{lf_consistency_patches.scene_01_shot_01.output_path}}\",\n"
        "    \"output_path\": null,\n"
        "    \"status\": \"pending\",\n"
        ...
        "  `lf_image`: \n"
        "    - If continuation_from_previous == false:\n"
        "      - If characters_present is not empty: `\"{{lf_consistency_patches.shot_id.output_path}}\"`\n"
        "        (the LF consistency-patch output — identity-corrected last frame that preserves the LF delta)\n"
        "      - If characters_present is empty: `\"{{lf_shots.shot_id.output_path}}\"`\n"
        "        (raw LF is fine — no character identity work to apply)\n"
        "    - If continuation_from_previous == true: `\"{{lf_shots.shot_id.output_path}}\"`\n"
        "      (Wave 2 shots skip LF consistency patches — the FF was extracted from the previous video, so identity is locked there)\n"
        "  `status`: 'pending'\n\n"
