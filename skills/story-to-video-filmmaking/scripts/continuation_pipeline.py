...
def generate_stitch_metadata(video_files, overlap_seconds=1.0, fps=25, output_json_path=None):
    ...
    ffmpeg_trim_cmd = f"ffmpeg {inputs_str} -filter_complex \"{filter_complex_str}\" -map \"[outv]\" stitched_output.mp4"
    ...
    if len(segments) >= 2:
        ...
        ffmpeg_xfade_cmd = f"ffmpeg {xfade_inputs.strip()} -filter_complex \"{xfade_filter}\" -map \"[outv]\" stitched_xfade_output.mp4"
    else:
